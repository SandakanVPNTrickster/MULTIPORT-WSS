...
surat=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/main/permission/ip | grep -w $ipes | awk '{print $4}')
...
fun_botOnOff() {
  ...
  [[ ! -f /root/multi/bot.conf ]] && {
    ...
    echo -ne "Bot Username, Dont use '@' [Ex: SquidBot] : "
    read bot_user
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/SandakanVPNTrickster/bot_panel/master/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
